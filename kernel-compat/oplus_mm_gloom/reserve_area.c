...
/* 不再需要高通的 android_debug_symbols：改为直接用内核导出的变量 */

#include "va_feature_hash.h"
#include "reserve_area.h"

#if IS_ENABLED(CONFIG_OPLUS_FEATURE_CPU_JANKINFO)
#include <../drivers/soc/oplus/cpu/sched/sched_info/osi_healthinfo.h>
#endif
...
{
	/* GKI 6.1 的 anon_vma_name() 返回内核内的 struct anon_vma_name，
	 * 名字就在 ->name[] 里，不再需要一加原版那套 get_user_pages_remote +
	 * kmap 翻页读用户空间的做法。 */
	...
	return snprintf(kbuf, klen, "[anon:%s]", anon_name->name);
}
