...
> [!WARNING]
> **4.1.1 必须成对刷入 Release 内的 `boot-ACLaniakea-SM8650Q-droidspaces-r3.img` 与 `vendor_boot-hyperSched-stub.img`。** 两者均已在 TB710FU 实机验证。本版 BSP 模块依赖 r3 才导出的符号，r2 内核上会加载失败。`vendor_boot-ColorOS16-original-TB710FU.img` 仅用于回退原始 ColorOS 引导链，不能与本项目的 boot 混刷。
>
...
不要将实验内核、旧 BSP 模块与 4.1.1 内核混用。启动后可用以下命令确认内核版本：
...
1. `FixModule-v4.1.1.zip`（内置 CryptoEng）
2. `OplusBSP-Modules-v4.1.1.zip`
3. 按需：`PenBridge-Module-v4.1.1.zip`
4. 按需：`SM8650Q-Scene-Scheduler-v4.1.1.zip`
5. 按需：`LenovoPadProGT-ZUI-Camera-Port-v4.1.1.zip`

命令行安装示例：

```bash
adb push FixModule-v4.1.1.zip /sdcard/
adb shell su -c 'ksud module install /sdcard/FixModule-v4.1.1.zip'
```

### 4. 安装并启用 LSPosed 模块

```bash
adb install -r BaseFix-Hook-v4.1.1.apk
adb install -r PenBridge-Hook-v4.1.1.apk       # 使用手写笔时
adb install -r PenHidCtl-v4.1.1.apk            # 使用手写笔时
adb install -r ZUI-Camera-Compat-v4.1.1.apk    # 使用 ZUI 相机时
```
...
完整 4.1.1 方案需要配套 boot、主修复、BaseFix Hook 和 OPlus BSP。手写笔、Scene 与 ZUI 相机均按需安装；CryptoEng 已内置于主修复。

**可以不刷 boot/vendor_boot 吗？**

不建议。4.1.1 的 BSP 依赖配套 boot 内核提供的符号和接口，也依赖成套阻塞版 `vendor_boot` 的启动配置。只想使用纯模块方案时请改用 v2.0.15。
