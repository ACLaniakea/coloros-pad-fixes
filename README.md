...
| 项目 | 类型 | 包名 / 模块 ID | 作用 |
| --- | --- | --- | --- |
| **修复模块**（基础修复+调优合并） | KernelSU 模块 | `coloros_port_fix` | AON QNN 生命周期、环境光自适应、杜比、HybridSwap 与六核性能/温控适配；小布 BWV 唤醒因联想 ADSP 模型不兼容而永久关闭并隐藏入口；使用原厂 nandswap/OSense 策略，仅做本机内存阈值、调度与内核节点兼容修正。 |
...
| **base-fix 基础修复** | LSPosed Hook | `com.aclaniakea.colorosostatsguard` | AON YUV 归一化、环境光色温桥接、杜比控制桥、电池健康、CPU/GPU 信息、OStats 日志防护、移植 Thermal HAL 的 skin 状态恢复；隐藏并停用不兼容的小布唤醒入口。 |
...
adb shell su -c 'ksud module install /sdcard/PenBridge-Module-v3.0.0.zip'
adb shell su -c 'ksud module install /sdcard/SM8650Q-Scene-Scheduler-v3.0.0.zip'

# 4. 安装 Hook APK（LSPosed）
adb install --no-incremental BaseFix-Hook-v3.0.0.apk
adb install --no-incremental -r PenBridge-Hook-v3.0.0.apk
...
- 小布 DSP（SoundTrigger/UIM）没有与联想 ADSP 兼容的模型，BWV 入口已默认关闭；若未来获得匹配模型，遗弃的实验代码可用于受控验证。
