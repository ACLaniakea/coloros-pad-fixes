...
# Lenovo hyperschedule HAL has no usable kernel endpoint on this port:
# /proc/lenovo_hyperschedule/{enable,static_ux,ux_period,ux_runtime} is absent.
# Leaving the Binder service alive cannot contribute a valid UX decision, while
# it can still issue failed requests during interactive transitions.  Do not
# unload lenovohyperSched itself: sched_walt has a hard module reference to it.
# This is a one-shot init control action, not a resident user-space bridge.
stop_lenovo_hyperschedule_hal() {
    [ -f "$MODDIR/disable-stop-lenovo-hyperschedule-hal" ] && {
        log_msg "lenovo-hyperschedule: stop suppressed by disable file"
        return 0
    }

    if [ -d /proc/lenovo_hyperschedule ]; then
        log_msg "WARN lenovo-hyperschedule: proc endpoint exists; HAL left running"
        return 0
    fi

    if [ "$(getprop init.svc.hyperschedule_hal_service)" = "running" ]; then
        setprop ctl.stop hyperschedule_hal_service
        sleep 1
    fi
    log_msg "lenovo-hyperschedule: HAL state=$(getprop init.svc.hyperschedule_hal_service); kernel module retained for sched_walt"
}
...
# HybridSwap 熄屏闸门
#
# 不保留用户态轮询。原生 notifier 重建须匹配 Lenovo provider 的 CRC；在此之前
# 只显式归零旧实验遗留的 pause，纯内核桥另行作为版本绑定模块试验。
# ============================================================================
_pn=/sys/block/zram0/hybridswap_swapd_pause
[ -w "$_pn" ] && echo 0 >"$_pn" 2>/dev/null
unset _pn
