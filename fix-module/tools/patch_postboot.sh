...
# fix-module/module/payload/bin/, where every swappiness write is 50 instead
# of 100. The module bind-mounts these copies over /vendor/bin at
# post-fs-data, so the boot scripts themselves write 50 (root-cause fix, no
# polling guard).
#
# Usage:
...
for name in init.qcom.post_boot.sh init.kernel.post_boot.sh; do
    ...
    sed 's/echo 100 > \/proc\/sys\/vm\/swappiness/echo 50 > \/proc\/sys\/vm\/swappiness/g' \
        "$src" >"$dst"
    chmod 0755 "$dst"
    echo "patched $dst"
done
