...
ui_print "- 8GB缓存进程上限默认48（persist.sys.aclaniakea.max_cached 可调），12GB保持原厂值"
ui_print "- 恢复内存扩展：以 Settings.Secure 为权威，开关与档位真实控制 zram"
ui_print "- 关闭 WLAN 固件诊断日志，省去约 1.3MB/s 的闪存写入"
ui_print "- 外挂 aclswap.ko：为联想内核补上 zram 写回，压缩池冷页可下沉到闪存"
