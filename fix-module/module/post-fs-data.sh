...
if [ -d "$AON_LIB_TARGET" ] && [ -f "$AON_LIB_PAYLOAD/libaiboost_jni.so" ] && \
   ...
   [ -f "$AON_LIB_PAYLOAD/cdsp/unsigned/libQnnHtpV75Skel.so" ]; then
    ...
    # The stock delegate begins ADSP_LIBRARY_PATH with an empty entry.  QNN
    # then fails before creating its HTP session on SM8650Q.  The payload
    # variant is a SHA-guarded one-byte correction that preserves the stock
    # search order while making its first entry absolute.
