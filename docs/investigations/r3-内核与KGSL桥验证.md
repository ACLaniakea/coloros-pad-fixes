...
> [!IMPORTANT]
> **最终结论（当日稍晚）：KGSL 桥已移除，配套内核退回 r2。**
>
> 判据是"原厂不驱动这套机制"：ROM 里引用 `kgsl/kgsl/proc` 的组件全是只读统计，
> 参照机同样如此且连 `state` 节点都没有。另有实测表明桥的待机收益要在下一次
> 解锁时偿还（桌面 95 分位帧时 24ms -> 101~121ms），而平板内核击杀记录为 0、
> 并不缺内存。
>
> r3 相对 r2 的配置差异只有一行 `CONFIG_LOCALVERSION`，唯一实质改动是为桥加的
> `filp_open`/`kernel_write` 符号导出；桥移除后无消费者，故退回 r2。
>
> 下面的排查过程仍然有效，尤其是 shrinker 那段 kprobe 定位。
