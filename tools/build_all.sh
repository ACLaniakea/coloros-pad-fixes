#!/usr/bin/env bash
# ACLaniakea ColorOS Pad Port Fixes - 4.0.5 全量构建
# 输出统一到 releases/。
#
...
cp releases/PenHidCtl-v4.0.5.apk pen-bridge/module/system/priv-app/aclpenhid/PenHidCtl.apk

echo "== 5/8 root module zips =="
python3 fix-module/tools/build_fix.py
python3 pen-bridge/module/tools/build_root.py pen-bridge/module releases/PenBridge-Module-v4.0.5.zip
...
cp zui-camera-compat-hook/releases/ZUI-Camera-Compat-v4.0.5.apk releases/
cp zui-camera-module/releases/LenovoPadProGT-ZUI-Camera-Port-v4.0.5.zip releases/

echo "== SHA256SUMS =="
( cd releases && sha256sum *-v4.0.5.* > SHA256SUMS-v4.0.5.txt )
