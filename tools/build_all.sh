#!/usr/bin/env bash
# ACLaniakea ColorOS Pad Port Fixes - 4.1.1 全量构建
# 输出统一到 releases/。
#
...
cp releases/PenHidCtl-v4.1.1.apk pen-bridge/module/system/priv-app/aclpenhid/PenHidCtl.apk

echo "== 5/8 root module zips =="
python3 fix-module/tools/build_fix.py
python3 pen-bridge/module/tools/build_root.py pen-bridge/module releases/PenBridge-Module-v4.1.1.zip
...
cp zui-camera-compat-hook/releases/ZUI-Camera-Compat-v4.1.1.apk releases/
cp zui-camera-module/releases/LenovoPadProGT-ZUI-Camera-Port-v4.1.1.zip releases/

echo "== SHA256SUMS =="
# 校验文件自身会被 *-v4.1.1.* 匹配到，列进去就永远 sha256sum -c 失败；排除掉。
# 同时带上配套内核件，它们没有版本后缀，glob 匹配不到。
( cd releases \
  && sha256sum $(ls *-v4.1.1.* | grep -v '^SHA256SUMS-') > SHA256SUMS-v4.1.1.txt \
